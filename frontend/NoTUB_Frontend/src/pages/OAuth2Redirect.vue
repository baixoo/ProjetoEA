    ...
    <q-dialog v-model="showProfileDialog" persistent :no-backdrop-dismiss="true" :no-route-dismiss="true">
      <q-card class="profile-dialog-card">
        <q-card-section>
          ...
          <p class="dialog-subtitle">Estes dados são necessários para o uso deste serviço, para mais informações, consulte a nossa política de privacidade.</p>
        </q-card-section>
        ...
        <q-card-actions align="right" class="column items-end q-pr-md q-pb-md">
          <q-btn
            label="Guardar"
            color="primary"
            :loading="saving"
          ...
          />
          <a href="#" class="skip-nif-link" @click.prevent="handleSkipNif">Não tenho NIF</a>
        </q-card-actions>
      </q-card>
    </q-dialog>
  </div>
...
async function handleSaveProfile() {
  ...
  try {
    ...
    profileErrors.nif = e.message || 'Erro ao guardar. Tente novamente.'
  } finally {
    saving.value = false
  }
}

async function handleSkipNif() {
  profileErrors.nif = ''
  profileErrors.dataNascimento = ''
  ...
  try {
    ...
    profileErrors.dataNascimento = 'Erro ao guardar. Tente novamente.'
  } finally {
    saving.value = false
  }
}
