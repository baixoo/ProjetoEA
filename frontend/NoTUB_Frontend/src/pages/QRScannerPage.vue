...
async function startScanner() {
  ...
  try {
    // Wait for the DOM element to exist just in case
    setTimeout(async () => {
      try {
        ...
        await html5QrcodeInstance.start(
          ...
          () => {
            // Silence frequent frame scan errors to prevent console spam
          }
        )
    ...
    console.error("Erro no setup da câmara:", err)
    scannerError.value = "Erro ao configurar a câmara"
    scannerRunning.value = false
  }
}
